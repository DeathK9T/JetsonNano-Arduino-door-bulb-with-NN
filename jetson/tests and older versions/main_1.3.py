...
class speechThread(Thread):
	'''self._words = {
		'open':['открой', 'открывай', 'откройся', 'открывайся'],
		'close':['закрой', 'закрывай', 'закройся', 'закрывайся'],
		'on':['включи', 'включай', 'включись', 'включайся'],
		'off':['выключи', 'выключай', 'выключись', 'выключайся']
	}'''
	net_detection = True
	speech = None
	def __init__(self):
	...
	def run(self):
		...
		speech.get_ambient()
		while True:
			self.speech = speech.run()
			print("You said", self.speech)
			if self.speech is None:
				print("Wouldn't get it.")
			elif "распознавание" in self.speech.lower() and "камеры" in self.speech.lower():
				if "включить" in self.speech.lower():
					if self.net_detection:
						print("Already on")
					else:
						self.net_detection = True
				elif "Выключить" in self.speech.lower():
					if self.net_detection:
...
def main():
	...
	while True:
		if speech_thread.net_detection:
			...
			for word in words['open']:
				if word in speech.statement:
					#playsound(mp3/Open.mp3)
					if s.write(129):
						opened = True

			for word in words['close']:
				if word in speech.statement:
					#playsound(mp3/Bye.mp3)
					if s.write(128):
						opened = False

			for word in words['on']:
				pass

			for word in words['off']:
				pass
