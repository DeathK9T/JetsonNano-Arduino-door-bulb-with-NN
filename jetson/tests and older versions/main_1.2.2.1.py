...
# parse the command line
	...
	while True:
		...
		if exist == "forward":
			if not opened:
				...
				if not old_found[1] and old_found[0]:
					...
					if time.time() - forward_cam.time_of_lost > 30:
						if s.writeCmd(128):
							opened = False
			...
			if not opened:
				...
				if not old_found[0] and old_found[1]:
					...
					if time.time() - backward_cam.time_of_lost > 30:
						#play_sound()
						if s.writeCmd(128):
							opened = False
		'''else:
		...
		'''if not opened:
			if old_found[0] or old_found[1]:
				if s.writeCmd(129):
					opened = True 
		else:
			if not old_found[0] and not old_found[1]:
				if s.writeCmd(128):
					opened = False'''	
		...
		'''if not opened:
			...
			if old_found[0] and old_found[1]:
				pass
			else:
				if s.writeCmd(128):
					opened = False'''
