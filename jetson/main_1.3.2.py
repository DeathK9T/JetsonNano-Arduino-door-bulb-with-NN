...
class speechThread(Thread):
	...
	def run(self):
		...
		while True:
			...
			elif "голос" in self.speech:
				if self.net_detection:
					print("Speech mode on")
					self.net_detection = False
					playsound("mp3/SpeechOn.mp3")
					continue
				else:
					print("Speech mode already on")
					playsound("mp3/SpeechAlreadyOn.mp3")
					continue
			elif "камера" in self.speech or "kamera" in self.speech or "camera" in self.speech:
				if not self.net_detection:
					print("Camera mode on")
					self.net_detection = True
					playsound("mp3/CameraOn.mp3")
					continue
				else:
					print("Camera mode already on")
					playsound("mp3/CameraAlreadyOn.mp3")
					continue
